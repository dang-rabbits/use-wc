    ...
    docs: {
      description: {
        component: [
          ...
          "",
          'Attributes drive the flex container: `direction` (`column` by default, or `row`), `align`, `justify`, `gap` (a named scale, `xsmall`…`super`), `padding`/`padding-block`/`padding-inline` (the same named scale, plus `none` for an explicit zero), `wrap`, and `inline`. `padding` also takes two tokens (`padding="medium small"`, block then inline like CSS\'s own shorthand) — that compound form is parsed by the element itself, so it needs `use-layout` to be upgraded; a single token works with or without it.',
          "",
